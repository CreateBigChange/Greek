...
//                }
     ...
     *
     */
    public function uploadApkQiniu(Request $request){
    if($request->hasFile('download_file')){
        $filename			= time() + mt_rand(1000 , 9999);

        $exten = substr($request->file('download_file')->getClientOriginalName()  , strpos($request->file('download_file')->getClientOriginalName() , '.' ) + 1 );


        if( $request->file('download_file')->isValid() ){
            // 要上传文件的本地路径
            $path = $request->file('download_file')->getPathname();
            ...
            // 生成上传 Token
            $token = $auth->uploadToken('jisxu-app-apk');
            ...
            if ($err !== null) {
                ...
                $ret['host'] = 'http://o9z0r6vs0.bkt.clouddn.com';
                return response()->json( Message::setResponseInfo( 'SUCCESS'  , $ret) );
            }
//                if( $request->file('img')->move($destinationPath , $filename . '.' . $exten )  ){
//                    $filePath = 'http://'.$_SERVER['SERVER_NAME'].'/upload_qiniu/'.$filename.'.'.$exten;
